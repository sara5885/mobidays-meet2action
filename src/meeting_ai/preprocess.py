...
ABBREV: dict[str, str] = {
    ...
    "GA4": "구글 애널리틱스 4(GA4)",
    "SA": "검색광고(Search Ads)",
    "GFA": "구글 디스플레이/파인드 광고(GFA)",
    "UTM": "캠페인 추적 파라미터(UTM)",
    "어트리뷰션": "전환 기여 분석(attribution)",
    "리타게팅": "재방문자 대상 광고(retargeting)",
    "룩어라이크": "유사 타겟 확장(lookalike)",
    "퍼스트파티": "자사 보유 데이터(1st-party data)",
    "프리퀀시": "1인당 노출 빈도(frequency)",
    "뷰어빌리티": "광고 노출 가시성(viewability)",
    "인스트림": "영상 중간 삽입 광고(in-stream)",
    "숏폼": "짧은 세로형 영상(short-form)",
    "소재 피로도": "동일 소재 반복 노출에 의한 성과 저하(creative fatigue)",
    "키비주얼": "캠페인 핵심 비주얼(key visual)",
    "톤앤매너": "브랜드 표현 톤·스타일(tone & manner)",
    "비딩": "입찰(bidding)",
    "인벤토리": "광고 지면/재고(inventory)",
}
